...
<div class="input-group mb-3">
  <span class="input-group-text" id="basic-addon1">Search User: </span>
  <input type="email" name="searchemail" class="form-control" placeholder="Email"  >
<input type="submit" name="searchuser" value="Search" class="btn btn-info">
...
if (mysqli_num_rows($result) > 0) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
   echo '    <tr>
      ...
      <td>'.$row["name"].'</td>
      <td>'.$row["email"].'</td>
      <td><a href="./edit.php?id='.$row["id"].'"><button class="btn btn-success">Edit</button></a>
	  
	  
	  <a href="./delete.php?id='.$row["id"].'"><button class="btn btn-danger">Delete</button></a></td>
    </tr>';
  }
}
